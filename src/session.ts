...
export class RTSession {
  ...
  constructor(
    ...
    this.logger.info(
      { message: this.systemMessage.message },
      "✅ Init message received",
    );
    this.initialize().catch((error) =>
      this.logger.error({ error }, "🔥 Failed to initialize session"),
    );
  }
  ...
  private async initializeRealtimeAzureOpenAIWebSocket(): Promise<WebSocket> {
    ...
    return new Promise(async (resolve, reject) => {
      const openAISocketClient =
        await OpenAIRealtimeWS.azure(azureOpenAIClient);
      ...
      openAISocketClient.socket.on("error", (error) => {
        this.logger.error(
          { error },
          `🔥 OpenAI WebSocket error: ${openAISocketClient.socket.url}`,
        );
        reject(error);
      });
    });
  ...
  private async getWebSocketHeaders(): Promise<{ [key: string]: string }> {
    if (BACKEND === "azure") {
      ...
      if (
        ...
        RTSession.tokenCache.expires >
          now + RTSession.TOKEN_REFRESH_THRESHOLD_MS
      ) {
  ...
  private setupEventHandlers() {
    this.clientWs.on("message", (data: RawData, isBinary) =>
    ...
    this.clientWs.on("error", (error) =>
      this.logger.error({ error }, "🔥 Client websocket error occurred"),
    );
  ...
  private handleRealtimeMessage(data: RawData) {
    try {
      ...
      const handlerMap: Record<string, (event: any) => void> = {
        ...
        [REALTIME_SERVER_EVENTS.ConversationItemInputAudioTranscriptionCompleted]:
          (event) =>
            this.logger.debug(
        ...
        [REALTIME_SERVER_EVENTS.ConversationItemInputAudioTranscriptionFailed]:
          (event) =>
            this.logger.error(
              { error: event.error },
  ...
  private sendInitialGreeting() {
    if (this.openAIWs?.readyState === WebSocket.OPEN) {
      this.logger.info(
        `🗣️ Sending initial greeting for ${this.systemMessage?.type}`,
      );
  ...
  private handleClientBinaryMessage(data: RawData) {
    ...
    if (!this.audioBufferTimer) {
      this.audioBufferTimer = setTimeout(
        () => this.flushAudioBuffer(),
  ...
  private flushAudioBuffer() {
    ...
    try {
      ...
      const bufferedAmount =
        (this.openAIWs as any)._socket?.bufferedAmount || 0;
      ...
      if (combinedBuffer.length === 0) {
        this.logger.warn(
          "Combined buffer is empty after combination. Skipping send.",
        );
        this.clearAudioQueue();
        return;
      }
  ...
  private handleBackpressure(bufferedAmount: number): boolean {
    ...
    const effectiveQueueSize = Math.max(
      1,
      Math.floor(this.MAX_QUEUE_SIZE * (1 - severityFactor)),
    );
    ...
    if (this.audioBufferQueue.length > effectiveQueueSize) {
      ...
      this.currentBufferSize = this.audioBufferQueue.reduce(
  ...
  private sendAudioToOpenAI(buffer: Buffer, totalSize: number): void {
    ...
    if (audioData.length > 5_000_000) {
      this.logger.debug(
        `Sending large audio payload: ${(audioData.length / 1024 / 1024).toFixed(2)}MB`,
      );
    }
  ...
  private updateAudioMetrics(size: number, duration: number): void {
    ...
    this.audioMetrics.maxBatchSize = Math.max(
      this.audioMetrics.maxBatchSize,
      size,
    );
  ...
  private updateLatencyMetrics(): void {
    ...
    if (this.audioMetrics.lastResponseTime > 0) {
      ...
      this.audioMetrics.avgLatency =
        (this.audioMetrics.avgLatency * (this.audioMetrics.totalResponses - 1) +
          latency) /
        this.audioMetrics.totalResponses;
    }
  ...
  private logAudioMetrics() {
    if (this.audioMetrics.totalBatchesSent > 0) {
      const sessionDuration =
        (Date.now() - this.audioMetrics.sessionStartTime) / 1000;
      const metrics = {
      ...
      this.logger.info(
        { audioStats: metrics },
        "Enhanced audio transmission statistics",
      );
    }
  }
  ...
  private handleFunctionCallArgumentsDone({
    ...
    try {
      ...
      if (!args) {
        this.logger.warn(
          { call_id },
          "🟠 No arguments provided in function call arguments done event",
        );
        return;
      }

      // Handle car control function call
      if (name === "send_car_commands") {
        try {
          ...
          // DO NOT trigger response.create here - it causes infinite loop!
          // The AI should only respond when the user speaks or sends a message
          ...
          this.logger.error(
            { error: parseError, args },
            "🔥 Failed to parse car commands",
          );
        }
        return;
      }
      ...
      this.logger.error(
        { error, call_id },
        "🔥 Error processing completed function call arguments",
      );
    }
  }
  ...
  private close() {
    ...
    if (
      this.clientWs &&
      this.clientWs.readyState !== WebSocket.CLOSED &&
  ...
  dispose() {
    ...
    if (
      ...
      try {
        this.logger.debug(
          `Flushing ${this.audioBufferQueue.length} remaining audio chunks (${this.currentBufferSize} bytes) before disposal`,
        );
        this.flushAudioBuffer();
      } catch (error) {
